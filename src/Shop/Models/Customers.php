...
class Customers extends \Users\Models\Users
{
	 ...
	 */
	public function primaryAddress($type='billing')
	{
		$item = null;

		switch ($type) {
			case "shipping":
				if ($items = (new \Shop\Models\CustomerAddresses)->setState('filter.user', (string) $this->id)->setState('filter.primary_shipping', true)->getItems())
				{
					$item = $items[0];
				}
				...
				break;
			default: // billing
				if ($items = (new \Shop\Models\CustomerAddresses)->setState('filter.user', (string) $this->id)->setState('filter.primary_billing', true)->getItems())
				{
					$item = $items[0];
				}
