  ...
  class Packer implements LoggerAwareInterface {
    ...
    public function doVolumePacking() {
      ...
      while ($this->items->count()) {
        ...
        /*
         * IN the case that an Item is too big to fit into any box, like a big exaust, we just quickly create a box 
         * the size of the product and pack it. That way we get the same list of Packages, for each package we send to ConnectShip
         *  
         */
        if ($packedBoxesIteration->isEmpty()) {
        	
        	//ITEM IS TOO BIG TO FIT IN ANY BOX
        	//MAYBE WE HAD A BOX WITH THE DIMS?
        	$item = $this->items->extract();
        	$box = \RallyShop\Models\Packages::fromCartItem($item); 
        	$list = new \RallyShop\Models\Packer\ItemList();
        	$list->insert($item);
        	$packedBox =  new PackedBox($box, $list, 0, 0, 0, 0);
